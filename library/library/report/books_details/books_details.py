...
def get_data(filters):
    ...
    if(filters.get('title')):conditions += f" AND title = '{filters.get('title')}'"
    if(filters.get('status')):conditions += f" AND status ='{filters.get('status')}'"
    
    
    data = frappe.db.sql("""SELECT name,title,authors,publisher,book_rent,status, COUNT(book_rent) OVER (PARTITION BY title) AS total_book  FROM `tabBooks`  ;""")
    return data
